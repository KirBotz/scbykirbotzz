...
const { getRandom } = require('../V2/functions')
...
exports.getBuffer = getBuffer = async (url) => {
    ...
    const anu = fs.readFileSync('./V4/fakeakira.jpeg')
...
exports.createExif = (pack, auth) =>{
    ...
    fs.writeFileSync('./V5/data.exif', Buffer.concat([_, __, ___, ____]), function (err) {
        console.log(err)
...
exports.modStick = (media, client, mek, from) => {
    ...
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './V5/data.exif', media, '-o', out])
        .on('exit', () => {
